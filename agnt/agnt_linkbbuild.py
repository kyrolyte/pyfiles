...
def format_chapter_link(book, chapter):
    book_url = book.lower().replace(" ", "-")
    return f"[{book} {chapter}](/bible/bsb/{book_url}/chapter-{chapter}/)"
...
for month_name, days_in_month in months:
    ...
    for day in range(1, days_in_month + 1):
        ...
        if not day_reading:
            ...
            # Group by book to maintain the "Book 1-3" style, but now each chapter is a link
            grouped = {}
            ...
            for b in grouped:
                chaps = grouped[b]
                links = [format_chapter_link(b, c) for c in chaps]
                book_readings.append(", ".join(links))
            
            reading_str = ", ".join(book_readings)
